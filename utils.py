...
class AssetFolder:
    ...
    def get_entries(self, include_root: bool, empty_too: bool) -> List[Tuple[str, str, str]]:
        ...
        for af in self.folders:
            entries = af.get_entries(include_root, empty_too)
            if len(entries) > 0:
                r.extend(entries)
                if self.depth == 0 and PreferencesPanel.get().separate_categories:
                    r.append(None)
...
def formats_to_parse(asset_type: str) -> List[str]:
    ...
    if asset_type == ASSET_TYPE_OBJECT:
        ...
        extensions.clear()
        if preferences.show_blend:
            extensions.append('.blend')
        if preferences.show_fbx:
...
class CategoriesCache:
        ...
        if os.path.exists(path):
            ...
            for e in sorted(os.listdir(path)):
                ...
                if os.path.isdir(abs_path) and not e.startswith('.'):
                    ...
                    # Do this recursively
                    asset_folder.add_folder(CategoriesCache.rec_scan_structure(
                        asset_type, rel_path, depth + 1))
    ...
    def update_cache(asset_type):
        CategoriesCache.cache[asset_type] = CategoriesCache.rec_scan_structure(
            asset_type)
...
def split_entry(entry_name):
    ...
    if "::" in entry_name:
        ...
        preview = os.path.splitext(
            imp)[0] + "__" + bpy.path.clean_name(mat) + ".png"
